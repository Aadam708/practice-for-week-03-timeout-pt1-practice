...
function batchTimeouts(callbacks, delays, arr=[], index=0) {
  // Your code here

  if (index === callbacks.length) return arr


  let cb = callbacks[index]
  let delay = delays[index]

  let res = setTimeout(cb, delay)

  arr.push(res)

  return batchTimeouts(callbacks,delays,arr,index+1)
}
